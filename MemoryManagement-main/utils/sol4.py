...
class Simulator:
    ...
    def initialize(self, workload_id):
        ...
        for _, request in workload.iterrows():  
            ...
            request_time -= config.window
            app.prt.append(request_time )             
            event = Event(request_time, EventTypes.STARTED, app)
            config.event_queue.add_event(event)
        
        for app in config.apps:
            app.nxt_rq = app.prt[0]
    ...
    def candidates(self):
        ...
        for app in  config.apps:
            if app.status != AppStatus.AGGRESSIVE and app.loaded_model_size > 0.0: 
                print(app.name, app.nxt_rq) 
                if app.nxt_rq > (config.time.get_time() + 2 * config.window):              
                    candids.append(app)
            # print([x.name for x in candids])
        return candids    
