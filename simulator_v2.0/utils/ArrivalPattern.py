...
In simulation, the tasks are dynamically arrived to the system following
certain dstributions. More specifically, the arrival time distribution 
indicates the arrival time of a cerain number of tasks in a specific 
period of time.

For now, the arrival pattern could be generated by sampling from 
"normal", "uniforn", "exponential", or "spiky" distributions.
...
class ArrivalPattern:
    # Using this class, the arrival pattern (distribution) is generated
    #
    ...
    # system is no_of_tasks. Then, a sample of size no_of_tasks which is
    # drawn from the certain distribution is returned as arrival time 
    # distribution for those number of tasks.
