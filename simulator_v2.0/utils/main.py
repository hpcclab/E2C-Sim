...
for i in range(100, 130):
    ...
    while Config.event_queue.event_list and Config.available_energy > 0:
        ...
        elif event.event_type == EventTypes.COMPLETION:  # 3
    ...
    out = open('./results/RLS/results-summary.csv', 'w')
    header = ['Episode', 'total_no_of_tasks', 'mapped', 'offloaded', 'cancelled', 'Completion%', 'xCompletion%',
              'URG_missed', 'BE_missed', 'available_energy']
    writer = csv.writer(out)
    writer.writerow(header)
    ...
    row.append([i, total_no_of_tasks, total_assigned_tasks, Config.cloud.stats['offloaded_tasks'],
                len(scheduler.stats['dropped']),
