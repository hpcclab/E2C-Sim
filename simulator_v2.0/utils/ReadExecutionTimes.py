...
class ReadData:
    ...
    def read_tasks_size(self, task_type_id):
        # Here, the execution time of task type on a specific machine type
        # is read from the dataset. This function returns a list that 
        # contains the execution times.
        # the file name of the dataset must be in the format of 
        ...
        with open(path, 'r') as data_file:
            for line in data_file:
                tasks_size.append(float(line))

        return tasks_size
        
            

    def sampled_task_size(self, task_type_id, k=1):
        # Here, it takes task_type_id, machine_type and "k", then read the
        # execution times from the dataset and finally returns a sample of 
        # "k" execution times. 

        sampled_task_size = []
        tasks_size = self.read_tasks_size(task_type_id)

        for _ in range(k):
            sampled_task_size.append(random.choice(tasks_size))

        sampled_task_size = [round(x, 1) for x in sampled_task_size]  

        return sampled_task_size
