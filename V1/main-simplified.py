...
scenario = 'sc-4'
etc_folder = 'heterogeneous/1_75'
workloads = range(30)
etcs = range(30)

for etc_id in etcs:
    config.init()
    #path_to_etc = f'./task_machine_performance/{etc_folder}/etc-{etc_id}.csv'
    ...
    df = df.rename(columns = {'M0':'m1', 'M1':'m2', 'M2':'m3'})
    
    df.to_csv(f'/home/C00424072/Ali/Projects/E2C-Sim/V1/tasK_machine_performance/heterogeneous/1_75/etc-{etc_id}.csv')
    path_to_reports = f'./output/data/{scenario}/{etc_folder}/etc_{etc_id}'
    ...
    for workload_id in workloads:
        print(f'Scenario: {scenario} , ETC#: {etc_id} , Workload: {workload_id}')
        config.init()     
