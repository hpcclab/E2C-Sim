...
class ItemDockDetail(QMainWindow):
    ...
    def machine_etc(self, tt, mt):                
        ...
        # We do not need to show the path, the user should just be able to load the etc csv file
        # #------------ 
        # self.etc_entry = QLineEdit(self)
        # self.etc_entry.setStyleSheet("QLineEdit"
        #                 "{"
        #                 "background : white;"
        #                 "}")
        # self.etc_entry.setText(self.path_to_etc)
        self.etc_btn = QPushButton('Load ETC', self)
        self.etc_btn.clicked.connect(self.get_etc_file)
        #--------------
        ...
        self.etc_edit.clicked.connect(self.enable_etc_matrix)
        ...
        #--------------
        # self.etc_grid.addWidget(self.etc_entry)
        self.etc_grid.addWidget(self.etc_btn, 1+len(tt),(len(mt)/2)+1,1,len(mt)/2) # the cell position of the button should be explicitly declared in gridLayout
        #----------------     
    ...
    def set_mq(self):
        ...
        self.only_int.setRange(1, 2147483646) #maximum range?
        self.mq_size.setValidator(self.only_int)
        #-----------------------
    ...
    def set_bq(self):
        ...
        self.only_int = QIntValidator()
        self.only_int.setRange(1, 2147483646) #maximum range?
        self.bq_size.setValidator(self.only_int)
        #-----------
    ...
    def workload_data(self, enabled):
        ...
        # self.etc_grid.addWidget(self.etc_entry)
        self.etc_grid.addWidget(self.etc_btn) 
        #---------------------------------------------------------------------
        ...
        #----------------------------------------
        self.tab_workload.layout.addLayout(self.etc_grid)                
        self.tab_workload.layout.addStretch(1)
        self.tab_workload.setLayout(self.tab_workload.layout)
        self.dock.setWidget(self.tabs)
        #---------------------------------------

        self.workload_grid.addWidget(self.path_entry,1,0)        
        self.workload_grid.addWidget(self.browse_btn,1,1) 
    ...
    def get_workload_file(self): 
        ...
        msg.setText("Workload .csv format is incorrect")
        msg.setWindowTitle("Warning MessageBox")
        msg.setStandardButtons(QMessageBox.Ok)

        try:   
            df = pd.read_csv(self.workload_path)

            if(len(df.columns)) != 2 or df.dtypes["arrival_time"] != "float64" or df.isnull().values.any():
                msg.exec_()
                return True
    ...
    def get_etc_file(self):     
        loaded_path  = QFileDialog.getOpenFileName(self, caption='Choose ETC File',
                                                    directory=QDir.currentPath(),
                                                    filter='*.csv')
        if loaded_path[0]:
            self.path_to_etc = loaded_path[0]
        self.etc_entry.setText(self.path_to_etc)


        msg = QMessageBox()
        msg.setIcon(QMessageBox.Warning)
        msg.setText("ETC .csv format is incorrect")
        msg.setWindowTitle("Warning MessageBox")
        msg.setStandardButtons(QMessageBox.Ok)

        try:
            df = pd.read_csv(self.path_to_etc)
            i = 0

            if(df.columns[0] == "idx"):    #1_75 csv format
                for col in df.iloc[:,1:]:
                    if(col[0].lower() != "m" or (col[1] != str(i+1) and col[1] != str(i)) or df.isnull().values.any() or (df.dtypes[col] != "float64" and df.dtypes[col] != "int64") or df["idx"].duplicated().any()):
                        msg.exec_()
                        return
                    i += 1
            else:
                for col in df:
                    if(col[0].lower() != "m" or col[1] != str(i) or df.isnull().values.any() or (df.dtypes[col] != "float64" and df.dtypes[col] != "int64")):
                        msg.exec_()
                        return
                    i += 1
                
            print("ETC passed format check")
            with open(self.path_to_etc,'r') as etc:
                etc_reader = csv.reader(etc)     
                next(etc_reader)        

                for idx, row in enumerate(etc_reader):   
                    self.etc_matrix.setRowCount(idx+1)

                    it = self.etc_matrix.verticalHeaderItem(idx)
                    if it is None:
                        val = self.etc_matrix.model().headerData(idx, Qt.Horizontal)
                        it = QTableWidgetItem(str(val))
                        self.etc_matrix.setVerticalHeaderItem(idx, it)
                    it.setText("T"+str(idx+1))

                    if(df.columns[0] == "idx"):  #1_75 csv format
                        for j, m in enumerate(row[1:]):
                            rounded_etc = round(float(m), 3)
                            cell_item = QTableWidgetItem(str(rounded_etc))
                            self.etc_matrix.setItem(idx, j, cell_item)
                    else:
                        for j, m in enumerate(row[0:]):
                            rounded_etc = round(float(m), 3)
                            cell_item = QTableWidgetItem(str(rounded_etc))
                            self.etc_matrix.setItem(idx, j, cell_item)
        except: msg.exec_()

        # task_type.setFlags(task_type.flags() ^ Qt.ItemIsEditable)
        # m1.setFlags(m1.flags() ^ Qt.ItemIsEditable)
        # m2.setFlags(m2.flags() ^ Qt.ItemIsEditable)
        # m3.setFlags(m3.flags() ^ Qt.ItemIsEditable)