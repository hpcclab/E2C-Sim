...
class Simulator(QObject):
    ...
    def run(self):
        ...
        if config.gui :
            ...
            with open(f'{self.path_to_reports}/{config.scheduling_method}/results-summary.csv','w')  as report_summary:
                report_header = ['workload_path', 'total_no_of_tasks','mapped','cancelled','URG_missed','BE_missed','Completion%','xCompletion%','totalCompletion%','throughput','utilization','wasted_energy%','consumed_energy%','energy_per_completion%']
    ...
    def report(self,is_detailed = True):
        ...
        for machine in config.machines:
            ...
            utilizations.append(100*machine.busy_time / config.time.gct())
            throughputs.append((machine.stats['completed_tasks'] + machine.stats['xcompleted_tasks']) / config.time.gct())
        ...
        total_utilization = sum(utilizations) / len(utilizations)
        ...
        row.append(
            ...
            total_completion_percent+total_xcompletion_percent,
            total_throughput, total_utilization,
