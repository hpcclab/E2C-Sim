...
class Simulator(QObject):    
    ...
    def __init__(self, heterogeneity_folder, etc_name,scenario, workload_id, seed=1):     
        super(Simulator, self).__init__()           
        self.path_to_etc = f"./heterogeneity/{heterogeneity_folder}/{etc_name}.csv" 
        self.path_to_arrival = f"./workloads/{scenario}/workload-{workload_id}.csv" 
        self.path_to_results = f"./output/data/{heterogeneity_folder}/{scenario}/{etc_name}" 
        self.etc_name = etc_name
        self.heterogeneity_folder = heterogeneity_folder
        self.seed = seed
    ...
    def create_event_queue(self):        
        arrivals = pd.read_csv(self.path_to_arrival) 
        etc = pd.read_csv(self.path_to_etc,index_col=0)
        print(etc)
        execution_time = ExecutionTime(self.heterogeneity_folder, self.seed)
        with open(f'./heterogeneity/{self.heterogeneity_folder}/execution_times.csv','w') as et_file:
    ...
    def run(self):        
        ...
        print(f'batch queue size : {self.scheduler.batch_queue.qsize()}')
