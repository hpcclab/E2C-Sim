...
class WFCFS(BaseScheduler):
    
    
    def __init__(self, total_no_of_tasks):
        super(WFCFS, self).__init__()
        self.name = 'WFCFS'
    ...
    def schedule(self):
        ...
        if self.batch_queue.empty():
            return None
        print(f'index: {self.prev_assignment_idx} Assigned: {self.assigned}')
        if self.assigned < self.priority[self.prev_assignment_idx]:
            machine_index = self.prev_assignment_idx
            self.assigned += 1
        else:
            self.assigned = 1              
